...
from tiro_fhir import Observation, Procedure, SCTConcept, SCTCoding, Reference, SCTFHIRTerminologyServer, SimpleValueSet
...
gp_observable = (SCTConcept("384994009 |Primary Gleason pattern|"), SCTConcept("384995005 |Secondary Gleason pattern (observable entity)|"))
gs_findings = [SCTConcept("18430005 |Gleason grade score 4|"), SCTConcept("46677009 |Gleason grade score 3|"), SCTConcept("84556003 |Gleason grade score 6|")]
gp_findings = [(SCTConcept("369771005 |Gleason Pattern 2|"), SCTConcept("369771005 |Gleason Pattern 2|")), (SCTConcept("369770006 |Gleason Pattern 1|"), SCTConcept("369771005 |Gleason Pattern 2|")), (SCTConcept("369772003 |Gleason Pattern 3|"), SCTConcept("369772003 |Gleason Pattern 3|"))]
dates = [datetime(2020,1,4), datetime(2018,4,28), datetime(2019,3,10)]
observations = [Observation(code=gs_observable, valueCodeableConcept=finding, effectiveDateTime=d, component=[{"code":  obs, "valueCodeableConcept": find} for (obs, find) in zip(gp_observable, comp_findings)]) for finding, comp_findings, d in zip(gs_findings, gp_findings, dates)]

# %%
subject = Reference(reference="test-patient")
procedures = [
    Procedure(code=SCTConcept("26294005+58347006 |Radical prostatectomy with lymphadenectomy|"), performedDateTime=datetime(2012, 11, 4), subject=subject),
    Procedure(code=SCTConcept("708919000 |RARP|"), performedDateTime=datetime(2015, 8, 10), subject=subject),
    Procedure(code=SCTConcept("90470006 |Prostatectomy|"), performedDateTime=datetime(2020, 4, 25), subject=subject),
    Procedure(code=SCTConcept("176258007 |Open prostatectomy|"), performedDateTime=datetime(2021, 7, 18), subject=subject)
]
# %%
...
df = store.Procedure["code", "performedDateTime"]
